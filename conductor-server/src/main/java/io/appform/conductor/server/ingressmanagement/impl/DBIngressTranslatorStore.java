...
public class DBIngressTranslatorStore implements IngressTranslatorStore {
    ...
    public Optional<IngressTranslator> read(@Throws.RuntimeParam("id") String id) {
        return ingressTranslatorLookupDao.get(id).map(this::toWire);
    }
    ...
    public Optional<IngressTranslator> createOrUpdate(IngressTranslator translator) {
        return ingressTranslatorLookupDao.createOrUpdate(
                        translator.getId(),
                        existing -> existing.setTemplate(translator.getTemplate())
                                .setDeleted(false),
                        () -> toStored(translator))
                .map(this::toWire);
    }
    ...
    private IngressTranslator toWire(StoredIngressTranslator translator) {
